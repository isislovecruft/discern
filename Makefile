...
# operating system:  Linux 2.6.38-11-generic x86_64 [ELF] (2.6.38)
# libc:	(6.13.0)
# binutils:	(221)
...
LOCAL_LIBRARIES = $(XAWLIB) $(XMULIBONLY) $(XTOOLLIB) $(XPMLIB) $(EXTRAXAWCLIENTLIBS) $(XLIB)
  SYS_LIBRARIES = -lm
           SRCS = globals.c main.c proc.c hfm.c trace.c lex.c 		  stats.c graph.c gwin.c

           OBJS = main.o proc.o hfm.o trace.o lex.o stats.o graph.o gwin.o

        PROGRAM = discern

all:: discern

discern: $(OBJS) $(DEPLIBS)
	$(RM) $@
...
install:: discern
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  discern $(DESTDIR)$(BINDIR)/discern

all:: discern.$(MANNEWSUFFIX)

discern.$(MANNEWSUFFIX): discern.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname discern` && \
	   $(LN) `basename discern.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < discern.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) discern.$(MANNEWSUFFIX)

install.man:: discern.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) discern.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/discern.$(MANSUFFIX)
...
install:: Discern.ad
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTAPPFLAGS) Discern.ad $(DESTDIR)$(XAPPLOADDIR)/Discern
